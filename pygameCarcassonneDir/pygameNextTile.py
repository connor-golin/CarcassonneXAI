...
FEATURE_DICT = {"C": "City", "R": "Road", "G": "Farm", "Monastery": "Monastery"}
...
class nextTile:
    ...
    def __init__(self, Carcassonne, displayScreen, RunInit=True, *args):
        ...
        if RunInit:
            ...
            # location
            self.X = (
                displayScreen.Total_Grid_Width
                + (displayScreen.Menu_Width - self.width) / 2
            )
            self.Y = 25
    ...
    def showNextTile(self, displayScreen, rotation, newRotation):
        ...
        # arrows
        pygame.draw.polygon(
            GAME_DISPLAY, left_color, ((x1, y1), (x1, y1 + 32), (x1 - 25, y1 + 16))
        )
        pygame.draw.polygon(
            GAME_DISPLAY, right_color, ((x2, y1), (x2, y1 + 32), (x2 + 25, y1 + 16))
        )
    ...
    def showInfos(self, displayScreen):
        ...
        GAME_DISPLAY.blit(
            self.meepleLabel, (Grid_Window_Width + (Menu_Width - width) / 2, 300)
        )
        GAME_DISPLAY.blit(
            self.moveLabel,
            (Grid_Window_Width + (Menu_Width - width) / 2, Grid_Window_Height - 340),
        )
    ...
    def evaluate_click(self, mouse_pos, displayScreen):
        ...
        """
        X, Y = get_clicked_X(mouse_pos, displayScreen), get_clicked_Y(
            mouse_pos, displayScreen
        )
        return X, Y
    ...
    def addMeepleLocations(
        self, location_key, Location, NumberKey, numberSelected, TileIndex
    ):
        """
        ...
        X, Y = meepleCoordinates(
            Location, Feature, MEEPLE_LOCATION_DICT_SCALED, TileIndex
        )
