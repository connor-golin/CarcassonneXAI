...
"""
SIDE_COMPARISON_DICT = {2: 0, 3: 1, 0: 2, 1: 3}
...
class CarcassonneState:
    ...
    def CloneState(self):
        ...
        Clone.BoardMonasteries = {
            k: v.CloneMonastery() for k, v in self.BoardMonasteries.items()
        }
        Clone.BoardFarms = {k: v.CloneFarm() for k, v in self.BoardFarms.items()}
        Clone.MonasteryOpenings = {
            k: [x for x in v] for k, v in self.MonasteryOpenings.items()
        }
    ...
    def completeMonastery(self, AffectedMonastery):
        ...
        if AffectedMonastery.Value == 9:
            ...
            self.FeatureScores[AffectedMonastery.Owner][
                2
            ] += 9  # update monastery feature score
            AffectedMonastery.Value = 0

    def checkMonasteryCompleteness(
        self, X, Y, SurroundingSpots, MeepleUpdate, MeepleKey=None
    ):
        """
        ...
        if (X, Y) in self.MonasteryOpenings:
            ...
            [
                (self.completeMonastery(self.BoardMonasteries[AffectedMonasteryIndex]))
                for AffectedMonasteryIndex in self.MonasteryOpenings[(X, Y)]
            ]
        ...
        if MeepleKey is not None:
            if MeepleKey[0] == "Monastery":
                ...
                CompleteSurroundingSpots = SurroundingSpots + [
                    (X - 1, Y - 1),
                    (X + 1, Y + 1),
                    (X + 1, Y - 1),
                    (X - 1, Y + 1),
                ...
                self.BoardMonasteries[NextMonasteryID] = Monastery(
                    NextMonasteryID, self.playerSymbol - 1
                )
                AffectedMonastery = self.BoardMonasteries[NextMonasteryID]
                [
                    self.monasterySurroundings(Spot, AffectedMonastery, NextMonasteryID)
    ...
    def checkCityCompleteness(
        self, PlayingTile, Surroundings, MeepleUpdate, MeepleKey=None
    ):
        """
    ...
    def checkRoadCompleteness(
        ...
        if PlayingTile.HasRoads:
            ...
            ClosingRoads = roadConnections(
                self, PlayingTile, Surroundings, ClosingRoads, MeepleUpdate, MeepleKey
            )
    ...
    def UpdateVirtualScores(self):
        ...
        cityP1 = sum(
            [
                v.Value
                for k, v in self.BoardCities.items()
                if v.Pointer == v.ID
        ...
        roadP1 = sum(
            [
                v.Value
                for k, v in self.BoardRoads.items()
                if v.Pointer == v.ID
                and v.Meeples[0] >= v.Meeples[1]
                and v.Meeples[0] > 0
            ]
        ...
        farmP1 = 3 * sum(
            [
                len([x for x in v.CityIndexes if self.BoardCities[x].ClosedFlag])
                for k, v in self.BoardFarms.items()
                if v.Pointer == v.ID
                and v.Meeples[0] >= v.Meeples[1]
                and v.Meeples[0] > 0
            ]
        ...
        while True:
            for i in range(4):
                ...
                if TestingTile is not None:
                    if (
                        TestingTile.Properties[(i + 2) % 4]
                        != EvaluatedTile.Properties[Sides[i]]
                    ):
                        IsTileFitting = False
                        break
            break
    ...
    def movesWithMeeples(
        ...
        if HasFeature:
            if Feature == "G":
                ...
                RotatedOpenings = [
                    [
                        i + SideChange if i + SideChange < 4 else i + SideChange - 4
                        for i in k
                    ]
                    for k in Openings
                ]
                ...
                if Feature == "G":
                    for FarmSide, FarmLine in Openings:
                        if self.Board.get(SurroundingSpots[FarmSide]) is None:
                            ...
                            MatchingFeature = self.matchingFeature(
                                EvaluatedTile,
                                self.BoardFarms,
                                Feature,
                                SurroundingSpots,
                            ...
                            if (
                                MatchingFeature.Meeples[0] > 0
                                or MatchingFeature.Meeples[1] > 0
                            ):
                                MeepleFitting = False
                                break
                else:
                    for Side in Openings:
                        if self.Board.get(SurroundingSpots[Side]) is None:
                            ...
                            if (
                                ...
                                break
                if MeepleFitting:
                    TempAvailableMoves.append(
                        AvailableMove(TileIndex, X, Y, Rotation, (Feature, i))
                    )
    ...
    def matchingFeature(
        ...
        # cities
        if Feature == "C":
            MatchingIndex = self.Board.get(SurroundingSpots[Side]).TileCitiesIndex[
                self.MatchingSide[Side]
            ]
        # roads
        elif Feature == "R":
            MatchingIndex = self.Board.get(SurroundingSpots[Side]).TileRoadsIndex[
                self.MatchingSide[Side]
            ]
        # farms
        else:
            MatchingIndex = self.Board.get(SurroundingSpots[Side]).TileFarmsIndex[
                self.MatchingSide[Side]
    ...
    def availableMoves(self, TilesOnly=False, TileIndexOther=None):
        ...
        allAvailableMoves = [
            (
                self.availableMovesForSpotRotations(
                    Spot, Rotation, EvaluatedTile, TileIndex, TilesOnly
                )
            )
    ...
    def availableMovesForSpotRotations(
        self, Spot, Rotation, EvaluatedTile, TileIndex, TilesOnly
    ):
        """
        ...
        IsTileFitting, SideChange = self.doesTileFit(
            EvaluatedTile, Rotation, SurroundingSpots
        )
        # TempAvailableMoves = []
        if IsTileFitting:
            ...
            if not TilesOnly:
                ...
                if self.Meeples[self.playerSymbol - 1] > 0:
                    ...
                    if EvaluatedTile.HasCities:
                        availableMoves = self.movesWithMeeples(
                            EvaluatedTile,
                            EvaluatedTile.HasCities,
                            EvaluatedTile.CityOpenings,
                            "C",
                            SideChange,
                            SurroundingSpots,
                            availableMoves,
                    ...
                    if EvaluatedTile.HasRoads:
                        availableMoves = self.movesWithMeeples(
                            EvaluatedTile,
                            EvaluatedTile.HasRoads,
                            EvaluatedTile.RoadOpenings,
                            "R",
                            SideChange,
                            SurroundingSpots,
                            availableMoves,
                    ...
                    if EvaluatedTile.HasFarms and not self.no_farms:
                        availableMoves = self.movesWithMeeples(
                            EvaluatedTile,
                            EvaluatedTile.HasFarms,
                            EvaluatedTile.FarmOpenings,
                            "G",
                            SideChange,
                            SurroundingSpots,
                            availableMoves,
                    ...
                    if EvaluatedTile.HasMonastery:
                        availableMoves.append(
                            AvailableMove(TileIndex, X, Y, Rotation, ("Monastery", 0))
                        )
